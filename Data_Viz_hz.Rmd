...
##Cleanning the 2017 China-US trade data
CN_2017 <-read_delim("/Users/sybilchoo/Downloads/SIPA 2023 Spring/Data Visualization/Final project/C_A_H5_156_2017.csv")
CN_2017_cleaned <- CN_2017 %>%
...
CN_US_2017 <- CN_2017_cleaned[CN_2017_cleaned$partnerCode == "842",]

##Cleanning the 2018 China-US trade data
CN_2018 <-read_delim("/Users/sybilchoo/Downloads/SIPA 2023 Spring/Data Visualization/Final project/C_A_H5_156_2018.csv")
CN_2018_cleaned <- CN_2018 %>%
  rename(HS6code = cmdCode) %>% 
  mutate(HS6code = as.factor(HS6code),
         flowCode = as.factor(flowCode),
         partnerCode = as.factor(partnerCode),
...
CN_US_2018 <- CN_2018_cleaned[CN_2018_cleaned$partnerCode == "842",]

##Cleanning the 2019 China-US trade data
CN_2019 <-read_delim("/Users/sybilchoo/Downloads/SIPA 2023 Spring/Data Visualization/Final project/C_A_H5_156_2019.csv")
CN_2019_cleaned <- CN_2019 %>%
  rename(HS6code = cmdCode) %>% 
  mutate(HS6code = as.factor(HS6code),
         flowCode = as.factor(flowCode),
         partnerCode = as.factor(partnerCode),
...
CN_US_2019 <- CN_2019_cleaned[CN_2019_cleaned$partnerCode == "842",]


##Cleanning the 2020 China-US trade data
CN_2020 <-read_delim("/Users/sybilchoo/Downloads/SIPA 2023 Spring/Data Visualization/Final project/C_A_H5_156_2020.csv")
CN_2020_cleaned <- CN_2020 %>%
  rename(HS6code = cmdCode) %>% 
  mutate(HS6code = as.factor(HS6code),
         flowCode = as.factor(flowCode),
         partnerCode = as.factor(partnerCode),
...
CN_US_2020 <- CN_2020_cleaned[CN_2020_cleaned$partnerCode == "842",]


##Cleanning the 2021 China-US trade data
CN_2021 <-read_delim("/Users/sybilchoo/Downloads/SIPA 2023 Spring/Data Visualization/Final project/C_A_H5_156_2021.csv")
CN_2021_cleaned <- CN_2021 %>%
  rename(HS6code = cmdCode) %>% 
  mutate(HS6code = as.factor(HS6code),
         flowCode = as.factor(flowCode),
         partnerCode = as.factor(partnerCode),
...
CN_US_2021 <- CN_2021_cleaned[CN_2021_cleaned$partnerCode == "842",]

##Commbined the past five years China-US data
combined_data <- rbind(CN_US_2017, CN_US_2018,CN_US_2019,CN_US_2020,CN_US_2021)

#Sepcify solar data between China and the US
filtered_data_US_solar <- combined_data[combined_data$HS6code %in% c("854140", "854150"),]

##Combine all the China import and export data
combined_data_CN <- rbind(CN_2017_cleaned, CN_2018_cleaned, CN_2019_cleaned,CN_2020_cleaned, CN_2021_cleaned)


filtered_data_CN_solar <- combined_data_CN[combined_data_CN$HS6code %in% c("854140", "854150","391000", "280461", "280469"),]
...
plot_list <- lapply(top_countries_list, function(df) {
  ...
  ggplot(df, aes(x = total_balance_primaryValue, y = PartnerCountry, fill = color)) +
    ...
    scale_x_continuous(limits = c(0, 450), breaks = seq(0, 400, by = 100))+
...
plot_grid(plotlist = plot_list, ncol = 4, align = "h") %>%
  ggdraw() +
  draw_label("The United States Top 5 Solar Panel Import Countries", fontface = "bold", x = 0.5, y = 0.69)
